...
from app.services.research.agent_v2 import run_agent_v2
...
@pytest.mark.asyncio
async def test_agent_v2_full_pass() -> None:
    ...
    assert result["plan"] == ["How does overlap affect recall?", "What chunk sizes work best?"]
    assert result["iterations_used"] == 1
    assert result["insights_used"] == 1
    assert result["steps"][-1]["node"] == "save_memory"


@pytest.mark.asyncio
async def test_agent_v2_reflect_revise() -> None:
    llm_service = AsyncMock()
    llm_service.chat.side_effect = [
        ...
        {"reply": '{"evaluation": "sufficient", "reason": "enough evidence"}'},
        {"reply": "# Report\n\nSummary"},
        {"reply": '{"verdict": "revise", "feedback": "Add limitations."}'},
    ...
    assert result["reflection_history"] == "Add limitations."
    assert [step["node"] for step in result["steps"]].count("revise_report") == 1


@pytest.mark.asyncio
async def test_agent_v2_with_memory() -> None:
    llm_service = AsyncMock()
    llm_service.chat.side_effect = [
        {"reply": '{"sub_tasks": ["How does overlap affect recall?"]}'},
        {"reply": "rag chunk overlap recall"},
...
@pytest.mark.asyncio
async def test_agent_v2_without_memory() -> None:
    llm_service = AsyncMock()
    llm_service.chat.side_effect = [
        {"reply": '{"sub_tasks": ["How does overlap affect recall?"]}'},
        {"reply": "rag chunk overlap recall"},
...
@pytest.mark.asyncio
async def test_agent_v2_max_iterations() -> None:
    llm_service = AsyncMock()
    llm_service.chat.side_effect = [
        {"reply": '{"sub_tasks": ["How does overlap affect recall?"]}'},
        {"reply": "rag chunk overlap recall"},
    ...
    assert result["iterations_used"] == 1
    assert result["steps"][-2]["node"] == "reflect"

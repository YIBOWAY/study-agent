...
现在先把范围说清楚：Phase R1 已经把课程入口和 Part 1 改成项目驱动学习，Phase R2 已经把 Part 2 改成同一套学习方式，Phase R3 已经把 Part 3 改成同一套学习方式，Phase R4 已经把 Part 4 改成同一套学习方式，Phase R5 已经把 Part 5 改成同一套学习方式，Phase R6 已经把 Part 6 改成同一套学习方式。Part 7 仍是当前 v1 材料，等 R7 继续改写后，才会完整符合新的 Part 教学结构。Capstone 现在还是占位页，完整项目练习会在 R8 补齐。也就是说，R1/R2/R3/R4/R5/R6 让你看清“终点是什么”以及“前六部分怎么学”，不是在宣称整套课程已经全部重写完成。
...
5. 继续 Part 2 到 Part 6 时，按 R2/R3/R4/R5/R6 的 lab 做完整练习；继续 Part 7 时，记住它还是 current v1 材料。
...
1. **Orient**：先看这个 Part 给本地论文研究助手增加什么能力，以及它现在是 R1/R2/R3/R4/R5/R6 rewritten 还是 current v1。
...
### Part 7: Current v1 Materials

这个文件现在可以读、可以做，但还没有完成 R7 的项目驱动重写。
...
实现状态和教学重写状态不是一回事。很多 runtime/product 代码已经存在，Parts 2-6 已完成 R2/R3/R4/R5/R6 教学重写，Part 7 也有 current v1 学习材料；但新的项目驱动教学法会继续通过 R7-R9 完成。
...
| R7 | 计划继续把 Part 7 改成同一套 Build -> Inspect -> Break -> Fix -> Reflect 学习结构 |
...
学习时用一个简单判断：如果你正在 Parts 1-6，就按 R1-R6 新结构认真做完整循环；如果你正在 Part 7，就把它当作有效的 current v1 材料，同时知道后续还会被重新组织成更强的项目驱动版本。

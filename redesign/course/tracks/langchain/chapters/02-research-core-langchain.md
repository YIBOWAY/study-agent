...
- **你会构建**：本地 paper fixtures → keyword retrieve → Evidence → Claim → ClaimLink。
- **你会解释**：为什么漂亮 final answer 不能替代证据链。
- **验收**：`uv run pytest packages/langchain_course/tests/test_research.py -q`。
- **诚实边界**：本轨检索是 **keyword**，不是 embedding RAG；live LLM 生成 claim 时仍应用结构校验，不信任散文。

## 与 handwritten 对照

| Handwritten | 本轨 |
| --- | --- |
| `Source` / `SourceIngestor` | `PaperDoc` + `default_paper_docs()` |
| `FakeRetriever` | `KeywordRetriever` |
